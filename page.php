...
if($temp == 'ajax'){
 ...
 if($temp == 'admin'){
	if($_SESSION['username']){
		...
		/* Redirect browser */
		if($_GET['type'] != 'login'){
			header("Location: " . $dir . "admin/login/");
			exit;
			}
...
if($temp != 'admin'){
 ...
 if($temp == 'admin'){
	$query = mysql_query("SELECT * FROM content WHERE page_id='$temp' AND page_num='$type'");
 };
